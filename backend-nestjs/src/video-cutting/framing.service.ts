...
export class FramingService {
   ...
   */
  private buildFilterComplex(options: FramingOptions): FilterResult | null {
    const {
      aspectRatio,
      paddingColor = '#000000',
      topCaption,
    ...
    if (aspectRatio && aspectRatio !== 'original') {
      dims = this.calculatePaddedDimensions(
        inputWidth,
        inputHeight,
        aspectRatio,
      );
      ...
      this.logger.debug(
        `Output: ${dims.outputWidth} x ${dims.outputHeight}`,
      );
    ...
    this.logger.debug(
      `Generated filter_complex: ${result.filterComplex}`,
    );
    return result;
  }
